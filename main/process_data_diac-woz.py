...
info = pd.read_csv(f"{path_to_data_dir}/{origin}/labels/all.csv")

# process and split (simple context-agnostic fractional splitting) data
data = []
pid = 1
for i in tqdm(info.index):
    file = f"{path_to_data_dir}/{origin}/Data/{info.loc[i, "Participant_ID"]}_P/{info.loc[i, "Participant_ID"]}_AUDIO.wav"
        ...
        if audio.duration_seconds < duration_min:
            ...
            for s in range(n_segments):
                ...
                data.append({
                    ...
                    "file_path": fname,
                    "sex": diacwoz_process_sex(info.loc[i, "Gender"]),
                    "phq-9": info.loc[i, "PHQ_Score"],
                    "gad-7": -1,
                })
